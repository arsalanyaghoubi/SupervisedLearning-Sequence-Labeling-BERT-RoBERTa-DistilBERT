...
def main():
    input_path = Path(r'D:\Brandeis University\Interview Challanges\CBH\Dataset\drugsCom_raw')
    output_path = Path(r'D:\Brandeis University\Interview Challanges\CBH')
    ...
    test.to_csv(output_path / 'test.tsv', sep='\t', encoding='utf-8', index=False, header=True, quoting=csv.QUOTE_NONE,
                escapechar=None)
    train.to_csv(output_path / 'train.tsv', sep='\t', encoding='utf-8', index=False, header=True,
                 quoting=csv.QUOTE_NONE, escapechar=None)
    dev.to_csv(output_path / 'dev.tsv', sep='\t', encoding='utf-8', index=False, header=True, quoting=csv.QUOTE_NONE,
               escapechar=None)
